...
class GeocodingService:
    ...
    def get_coordinates(self, city: str, country: str) -> Optional[Tuple[float, float]]:
        ...
        try:
            ...
            print(f'Error fetching coordinates for {city}, {country}: {exc}')
            return None
        except (ValueError, KeyError) as exc:
            print(f'Error parsing coordinates for {city}, {country}: {exc}')
            return None
        except Exception as exc:  # noqa: BLE001
            print(f'Unexpected error in geocoding: {exc}')
            return None
